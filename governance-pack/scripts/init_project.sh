...
usage() {
  cat <<'EOF'
Usage:
  ./governance-pack/scripts/init_project.sh <stack> [custom_init_command]
...
Examples:
  ./governance-pack/scripts/init_project.sh laravel
  ./governance-pack/scripts/init_project.sh nextjs
  ./governance-pack/scripts/init_project.sh flutter
  ./governance-pack/scripts/init_project.sh python "uv init \"$INIT_TARGET_DIR\""
EOF
}
...
run_default_init() {
  case "$STACK" in
    ...
    python)
      ...
      echo "Example: ./governance-pack/scripts/init_project.sh python \"uv init \\\"\\$INIT_TARGET_DIR\\\"\""
      exit 1
      ;;
    *)

...
- GitHub evaluates **`uses: ./…` against the caller workspace before steps run**, so local `./.github/actions/…` cannot point into another repo. **`steps.*.uses` also cannot reference `inputs`**, so `universal-ci.yml` loads **`setup-governance-pack`** / **`setup-runtime`** from a **literal** `hao47363/better-dev-ci/.../actions/...@stable`; forked tooling must replace those lines or vendor this workflow.
...
Pin `uses: …/universal-ci.yml@<full_commit_sha>` for immutability, stay on **`stable`** for continuous updates, or use **semver tags** only after review. See [Release/versioning](docs/governance/release-versioning.md) and [github-ci/README.md](github-ci/README.md).
...
Cross-repo reusable workflows treat **`uses: ./…`** as paths **inside the application repository**, and they are evaluated **before** steps run—so composites cannot be “checked out first” into a subfolder of the app workspace. **`universal-ci.yml`** loads **`setup-governance-pack`** / **`setup-runtime`** from a **literal** `hao47363/better-dev-ci/.../actions/...@stable` (GitHub forbids `inputs` in `steps.uses`). Pass **`tooling_repository`** and **`tooling_ref`** so the **scripts/** checkout inside those actions matches your tooling repo and pin. See [Centralized CI setup](docs/central-ci-setup.md#cross-repo-reusable-workflow-constraints).
